...
public class SwingListener16 implements Listener{
					...
					if(ClassicSword16.isCShield(p.getInventory().getItemInMainHand())){
						if(event.getPlayer().getGameMode().equals(GameMode.CREATIVE)) {
							...
							if(!Storage.instabreaks.contains(event.getBlock().getType())) {
								...
								ClassicSword16.damageCItem(p, csword, 2);
							}
						}
					}
				}
	...
	public void onPlayerHit(EntityDamageByEntityEvent event){
		...
		if(event.getEntity() != null){
			if(event.getDamager() instanceof Player) {
				...
				if(ClassicSword16.isCShield(weapon) && p.getGameMode() != GameMode.CREATIVE) {
					ClassicSword16.damageCItem(p, weapon, 1);
				}
			}
		}
	}
	...
	public void onHit(PlayerInteractEvent event){
		if(event.getPlayer() != null){
			...
			if((event.getAction() == Action.LEFT_CLICK_BLOCK) || (event.getAction() == Action.LEFT_CLICK_AIR)) {
				if(ClassicSword16.isCShield(event.getPlayer().getInventory().getItemInMainHand())) {
					Storage.swingers.add(event.getPlayer());
					BlockHittingMain.runBlockHit(event.getPlayer());
					if(!ClassicSword16.isBlocking(p)) {
						ClassicSword16.blockToShieldInv(p.getInventory());
					}
				}
			}

			if(event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
				if(Storage.swingers.contains(p) && ClassicSword16.isCShield(p.getInventory().getItemInMainHand())) {
					p.getInventory().setItemInMainHand(ClassicSword16.toBlockShield(p.getInventory().getItemInMainHand(), ItemType16.getType(ClassicSword16.getCShieldShortName(p.getInventory().getItemInMainHand()))));
				...
				if (ClassicSword16.isCItem(p.getInventory().getItemInMainHand())) {
			...
			try{
				...
				if(event.hasBlock()){
					if(holding != null){
						if(holding.hasItemMeta()){
							if(ClassicSword16.isCShield(holding)){
				...
				if(ItemType16.isSword(holding.getType())) {
					ClassicSword16.toShield(holding, ItemType16.getSwordType(holding.getType()));
				}
			}
